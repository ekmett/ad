...
-- 0.0 :+ 1.0
findZero :: (Fractional a, Eq a) => (forall s. Tower s a -> Tower s a) -> a -> [a]
findZero f = go where
...
inverse :: (Fractional a, Eq a) => (forall s. Tower s a -> Tower s a) -> a -> a -> [a]
...
-- 0.7390851332151607
fixedPoint :: (Fractional a, Eq a) => (forall s. Tower s a -> Tower s a) -> a -> [a]
...
extremum :: (Fractional a, Eq a) => (forall s s'. ComposeMode (Forward s) (Tower s' a) -> ComposeMode (Forward s) (Tower s' a)) -> a -> [a]
