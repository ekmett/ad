...
newtype Kahn s a = Kahn (Tape a (Kahn s a)) deriving (Show, Typeable)

type instance Scalar (Kahn s a) = a

-- deriving instance (Data (Tape a (Kahn a)) => Data (Kahn a)

instance MuRef (Kahn s a) where
    type DeRef (Kahn s a) = Tape a
...
instance Num a => Mode (Kahn s a) where
...
instance Num a => Primal (Kahn s a) where
    primal (Kahn Zero) = 0
...
instance Num a => Jacobian (Kahn s a) where
    type D (Kahn s a) = Id s a
...
let s = VarT (mkName "s") in deriveNumeric id (ConT ''Kahn `AppT` s) s

derivative :: Num a => Kahn s a -> a
derivative = sum . map snd . partials
{-# INLINE derivative #-}

derivative' :: Num a => Kahn s a -> (a, a)
...
{-# SPECIALIZE partials :: Kahn s Double -> [(Int, Double)] #-}
partials :: forall s a . Num a => Kahn s a -> [(Int, a)]
...
partialArray :: Num a => (Int, Int) -> Kahn s a -> Array Int a
partialArray vbounds tape = accumArray (+) 0 vbounds (partials tape)
{-# INLINE partialArray #-}

-- | Return an 'IntMap' of sparse partials
partialMap :: Num a => Kahn s a -> IntMap a
...
instance Num a => Var (Kahn s a) where
