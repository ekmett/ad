...
instance (Num a, Reifies s Tape) => Mode (Reverse s a) where
...
instance (Num a, Reifies s Tape) => Primal (Reverse s a) where
    primal Zero = 0
    primal (Lift a) = a
    primal (Reverse _ a) = a

instance (Reifies s Tape, Num a) => Jacobian (Reverse s a) where
    type D (Reverse s a) = Id s a
...
let s = VarT (mkName "s") in
  deriveNumeric (ClassP ''Reifies [s, ConT ''Tape] :) (ConT ''Reverse `AppT` s) s
...
instance (Num a, Reifies s Tape) => Var (Reverse s a) where
    var a v = Reverse v a
    varId (Reverse v _) = v
