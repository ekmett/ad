...
du :: (Functor f, Num a) => (forall s. f (AD Forward s a) -> AD Forward s a) -> f (a, a) -> a
...
du' :: (Functor f, Num a) => (forall s. f (AD Forward s a) -> AD Forward s a) -> f (a, a) -> (a, a)
...
duF :: (Functor f, Functor g, Num a) => (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f (a, a) -> g a
...
duF' :: (Functor f, Functor g, Num a) => (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f (a, a) -> g (a, a)
...
-- >>> diff sin 0
-- 1.0
diff :: Num a => (forall s. AD Forward s a -> AD Forward s a) -> a -> a
...
diff' :: Num a => (forall s. AD Forward s a -> AD Forward s a) -> a -> (a, a)
...
-- [1.0,-0.0]
diffF :: (Functor f, Num a) => (forall s. AD Forward s a -> f (AD Forward s a)) -> a -> f a
...
-- >>> diffF' (\a -> [sin a, cos a]) 0
-- [(0.0,1.0),(1.0,-0.0)]
diffF' :: (Functor f, Num a) => (forall s. AD Forward s a -> f (AD Forward s a)) -> a -> f (a, a)
diffF' f a = unbundle <$> apply f a
{-# INLINE diffF' #-}

-- | A fast, simple, transposed Jacobian computed with forward-mode AD.
jacobianT :: (Traversable f, Functor g, Num a) => (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f a -> f (g a)
...
jacobianWithT :: (Traversable f, Functor g, Num a) => (a -> a -> b) -> (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f a -> f (g b)
...
jacobian :: (Traversable f, Traversable g, Num a) => (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f a -> g (f a)
...
jacobianWith :: (Traversable f, Traversable g, Num a) => (a -> a -> b) -> (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f a -> g (f b)
...
jacobian' :: (Traversable f, Traversable g, Num a) => (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f a -> g (a, f a)
...
jacobianWith' :: (Traversable f, Traversable g, Num a) => (a -> a -> b) -> (forall s. f (AD Forward s a) -> g (AD Forward s a)) -> f a -> g (a, f b)
...
grad :: (Traversable f, Num a) => (forall s. f (AD Forward s a) -> AD Forward s a) -> f a -> f a
...
--
-- Note, this performs /O(n)/ worse than 'Numeric.AD.Mode.Wengert.grad'' for @n@ inputs, in exchange for better space utilization.
grad' :: (Traversable f, Num a) => (forall s. f (AD Forward s a) -> AD Forward s a) -> f a -> (a, f a)
...
gradWith :: (Traversable f, Num a) => (a -> a -> b) -> (forall s. f (AD Forward s a) -> AD Forward s a) -> f a -> f b
...
gradWith' :: (Traversable f, Num a) => (a -> a -> b) -> (forall s. f (AD Forward s a) -> AD Forward s a) -> f a -> (a, f b)
...
--
hessianProduct :: (Traversable f, Num a) => (forall s s'. f (AD (ComposeMode Forward Forward s') s a) -> AD (ComposeMode Forward Forward s') s a) -> f (a, a) -> f a
...
hessianProduct' :: (Traversable f, Num a) => (forall s s'. f (AD (ComposeMode Forward Forward s') s a) -> AD (ComposeMode Forward Forward s') s a) -> f (a, a) -> f (a, a)
hessianProduct' f = duF' $ grad $ decomposeMode . f . fmap composeMode
